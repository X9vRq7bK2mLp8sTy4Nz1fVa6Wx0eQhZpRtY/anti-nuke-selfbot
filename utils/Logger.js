...
class Logger {
  ...
  async sendDMToOwners(message) {
    if (!global.client || !global.config) return;
    try {
      const owners = [];
      if (global.config.selfbot?.owner1_id) owners.push(global.config.selfbot.owner1_id);
      if (global.config.selfbot?.owner2_id) owners.push(global.config.selfbot.owner2_id);
  ...
  warn(message) {
    ...
    this.sendDMToOwners(`[WARN] ${message}`);
  }
  raw(message) {
    console.log(message);
  }
