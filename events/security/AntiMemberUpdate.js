  ...
  async execute(client, oldMember, newMember) {
    ...
    try {
      ...
      if (updateEntry && updateEntry.executor) {
        ...
        await db.saveOriginalMember(guild.id, newMember.id, originalRoleIds);
        ...
        if (thresholdExceeded) {
          ...
          const recentUpdates = await db.getOriginalMembers(guild.id, timeWindow);
          ...
          if (executorPunished) {
            ...
            if (successCount > 0) await db.clearOriginalMembers(guild.id);
          }

          await AntiNukeManager.cleanupActionData(
            "memberUpdates",
            executor.id,
            guild.id,
          );
