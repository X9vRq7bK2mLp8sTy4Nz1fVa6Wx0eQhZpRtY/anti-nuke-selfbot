  ...
  async execute(client, ban) {
    ...
    try {
      ...
      if (banEntry && banEntry.executor) {
        ...
        await db.saveBannedUser(guild.id, bannedUser.id, executor.id);
        ...
        if (thresholdExceeded) {
          ...
          const recentlyBannedIds = await db.getBannedUsers(
            guild.id,
            executor.id,
            timeWindow,
          );
          ...
          if (executorPunished) {
            ...
            if (successCount > 0) await db.clearBannedUsers(guild.id, executor.id);
          }

          await AntiNukeManager.cleanupActionData("bans", executor.id, guild.id);
