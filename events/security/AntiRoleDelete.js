  ...
  async execute(client, role) {
    ...
    try {
      ...
      if (deleteEntry && deleteEntry.executor) {
        ...
        await db.saveDeletedRole(guild.id, role.id, roleMetadata);
        ...
        if (thresholdExceeded) {
          ...
          const userDeletedRoles = await db.getDeletedRoles(guild.id, timeWindow);
          ...
          if (executorPunished && AntiNukeManager.isRoleRecoveryEnabled()) {
            ...
            if (successCount > 0) await db.clearDeletedRoles(guild.id);
          }
