  ...
  async execute(client, channel) {
    ...
    try {
      ...
      if (deleteEntry && deleteEntry.executor) {
        ...
        await db.saveDeletedChannel(channel.guild.id, channel.id, channelMetadata);
        ...
        if (thresholdExceeded) {
          ...
          const userDeletedChannels = await db.getDeletedChannels(
            channel.guild.id,
            timeWindow
          );
          ...
          if (executorPunished && AntiNukeManager.isChannelRecoveryEnabled()) {
            ...
            if (successCount > 0) await db.clearDeletedChannels(channel.guild.id);
          }
