  ...
  async execute(client, ban) {
    ...
    try {
      ...
      if (unbanEntry && unbanEntry.executor) {
        ...
        await db.saveUnbannedUser(guild.id, unbannedUser.id, executor.id);

        const thresholdExceeded = await AntiNukeManager.recordAction(
          "unbans",
          executor.id,
          guild.id,
          false
        ...
        if (thresholdExceeded) {
          ...
          const recentlyUnbannedIds = await db.getUnbannedUsers(
            guild.id,
            executor.id,
            timeWindow
          );
          ...
          if (executorPunished) {
            ...
            if (successCount > 0) await db.clearUnbannedUsers(guild.id, executor.id);
          }

          await AntiNukeManager.cleanupActionData("unbans", executor.id, guild.id);
          await AntiNukeManager.markOperationComplete(executor.id);
        }
      }
    } catch (error) {
