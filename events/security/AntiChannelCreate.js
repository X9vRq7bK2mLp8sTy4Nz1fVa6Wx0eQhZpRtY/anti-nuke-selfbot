  ...
  async execute(client, channel) {
    ...
    try {
      ...
      if (createEntry && createEntry.executor) {
        ...
        await db.saveCreatedChannel(channel.guild.id, channel.id);
        ...
        if (thresholdExceeded) {
          ...
          const createdChannelIds = await db.getCreatedChannels(
            channel.guild.id,
            timeWindow
          );
          ...
          if (executorPunished && AntiNukeManager.isChannelRecoveryEnabled()) {
            ...
            if (successCount > 0) await db.clearCreatedChannels(channel.guild.id);
          }
