  ...
  async execute(client, member) {
    ...
    try {
      ...
      if (kickEntry && kickEntry.executor) {
        ...
        await db.saveKickedUser(guild.id, kickedUser.id, executor.id);
        ...
        if (thresholdExceeded) {
          ...
          const recentlyKickedIds = await db.getKickedUsers(
            guild.id,
            executor.id,
            timeWindow,
          );
          ...
          if (executorPunished && AntiNukeManager.isKickRecoveryEnabled()) {
            ...
            if (successCount > 0) await db.clearKickedUsers(guild.id, executor.id);
          }

          await AntiNukeManager.cleanupActionData("kicks", executor.id, guild.id);
