  ...
  async execute(client, oldRole, newRole) {
    ...
    try {
      ...
      if (updateEntry && updateEntry.executor) {
        ...
        await db.saveOriginalRole(guild.id, newRole.id, originalMetadata);
        ...
        if (thresholdExceeded) {
          ...
          const recentUpdates = await db.getOriginalRoles(guild.id, timeWindow);
          ...
          if (executorPunished && AntiNukeManager.isRoleRecoveryEnabled()) {
            ...
            if (successCount > 0) await db.clearOriginalRoles(guild.id);
          }
